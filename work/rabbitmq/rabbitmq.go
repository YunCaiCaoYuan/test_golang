...
func GetMqUrl(cfg *RabbitMqConfig) string {
	s := bytes.Buffer{}
	s.WriteString("amqp://")
	s.WriteString(url.QueryEscape(cfg.Username))
	s.WriteString(":")
	s.WriteString(url.QueryEscape(cfg.Password))
	s.WriteString("@")
	s.WriteString(cfg.Host)
	s.WriteString(":")
	s.WriteString(strconv.Itoa(cfg.Port))
	s.WriteString("/")
	s.WriteString(url.QueryEscape(cfg.Vhost))
	uri := s.String()
	return uri
}
...
func (m *MQ) keepalive() {
	...
	case err := <-m.closeC:
		if err == nil {
			log.Error("[ERROR] MQ: Disconnected with MQ, but Error detail is nil")
		} else {
			log.Error(fmt.Sprintf("[ERROR] MQ: Disconnected with MQ, code:%d, reason:%s\n", err.Code, err.Reason))
		}
		...
		for i := 0; i < maxRetry; i++ {
			time.Sleep(time.Second)
			if _, e := m.Open(); e != nil {
				log.Error(fmt.Sprintf("[ERROR] MQ: Connection recover failed for %d times, %v\n", i+1, e))
				continue
			}
			log.Info(fmt.Sprintf("[INFO] MQ: Connection recover OK. Total try %d times\n", i+1))
			return
		}
		log.Error(fmt.Sprintf("[ERROR] MQ: Try to reconnect to MQ failed over maxRetry(%d), so exit.\n", maxRetry))
	}
}
