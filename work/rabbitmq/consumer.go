...
func (c *Consumer) Open(opts ...CustomConsumeOption) error {
...
func (c *Consumer) keepalive() {
	...
	case <-c.stopC:
		// 正常关闭
		log.Warn(fmt.Sprintf("[WARN] Consumer(%s) shutdown normally\n", c.Name()))
		c.mutex.Lock()
		c.ch.Close()
		c.ch = nil
		c.state = StateClosed
	...
	case err := <-c.closeC:
		if err == nil {
			log.Error(fmt.Sprintf("[ERROR] MQ: Consumer(%s)'s channel was closed, but Error detail is nil\n", c.name))
		} else {
			log.Error(fmt.Sprintf("[ERROR] MQ: Consumer(%s)'s channel was closed, code:%d, reason:%s\n", c.name, err.Code, err.Reason))
		}
		...
		for i := 0; i < maxRetry; i++ {
			...
			if c.mq.State() != StateOpened {
				log.Warn(fmt.Sprintf("[WARN] MQ: Consumer(%s) try to recover channel for %d times, but mq's state != StateOpened\n", c.name, i+1))
				continue
			}
			if e := c.Open(); e != nil {
				log.Warn(fmt.Sprintf("[WARN] MQ: Consumer(%s) recover channel failed for %d times, Err:%v\n", c.name, i+1, e))
				continue
			}
			log.Info(fmt.Sprintf("[INFO] MQ: Consumer(%s) recover channel OK. Total try %d times\n", c.name, i+1))
			return
		}
		log.Error(fmt.Sprintf("[ERROR] MQ: Consumer(%s) try to recover channel over maxRetry(%d), so exit\n", c.name, maxRetry))
	}
}
