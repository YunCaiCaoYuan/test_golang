...
func (p *Producer) Close() {
	...
	// fix: Open 失败时，defer p.Close() 会 panic ，让组件调用者不知所云
...
func (p *Producer) keepalive() {
	...
	case <-p.stopC:
		// 正常关闭
		log.Warn(fmt.Sprintf("[WARN] MQ: Producer(%s) shutdown normally.\n", p.name))
		p.mutex.Lock()
		p.ch.Close()
		p.state = StateClosed
		p.mutex.Unlock()
	...
	case err := <-p.closeC:
		if err == nil {
			log.Error(fmt.Sprintf("[ERROR] MQ: Producer(%s)'s channel was closed, but Error detail is nil\n", p.name))
		} else {
			log.Error(fmt.Sprintf("[ERROR] MQ: Producer(%s)'s channel was closed, code:%d, reason:%s\n", p.name, err.Code, err.Reason))
		}
		...
		for i := 0; i < maxRetry; i++ {
			...
			if p.mq.State() != StateOpened {
				log.Warn(fmt.Sprintf("[WARN] MQ: Producer(%s) try to recover channel for %d times, but mq's state != StateOpened\n", p.name, i+1))
				continue
			}
			if e := p.Open(); e != nil {
				log.Warn(fmt.Sprintf("[WARN] MQ: Producer(%s) recover channel failed for %d times, Err:%v\n", p.name, i+1, e))
				continue
			}
			log.Info(fmt.Sprintf("[INFO] MQ: Producer(%s) recover channel OK. Total try %d times\n", p.name, i+1))
			return
		}
		log.Error(fmt.Sprintf("[ERROR] MQ: Producer(%s) try to recover channel over maxRetry(%d), so exit\n", p.name, maxRetry))
	}
}
...
func applyExchangeBinds(ch *amqp.Channel, exchangeBinds []*ExchangeBinds) (err error) {
	...
	for _, eb := range exchangeBinds {
		...
		needQueueBinding := true

		// 如果是 direct 或 fanout 模式，不需要强行绑定队列
		if eb.Exch.Kind == ExchangeDirect || eb.Exch.Kind == ExchangeFanout {
			needQueueBinding = false
		}

		if eb.Exch == nil {
			return errors.New("MQ: Nil exchange found.")
		}
		if needQueueBinding && len(eb.Bindings) <= 0 {
		...
		for _, b := range eb.Bindings {
			...
			if needQueueBinding && len(b.Queues) <= 0 {
